...
def body_pose_to_body_RTs(jangles, tpose_joints, use_smplx=True):
...
def body_pose_to_body_RTs_tensor(jangles, tpose_joints, use_smplx=True):
    r""" Convert body pose to global rotation matrix R and translation T.
...
def get_canonical_global_tfms(canonical_joints, use_smplx=True):
...
def get_joints_from_pose(poses, tpose_joints, use_smplx=True):
    if not use_smplx:
        PARENT = SMPL_PARENT
    else:
        PARENT = SMPLX_PARENT
...
def get_joints_from_RTs(tpose_joints, cnl_gtfms, dst_Rs, dst_Ts, use_smplx=True):
...
def get_global_RTs(cnl_gtfms, dst_Rs, dst_Ts, use_smplx=True):
    if not use_smplx:
        PARENT = SMPL_PARENT
    else:
        PARENT = SMPLX_PARENT
