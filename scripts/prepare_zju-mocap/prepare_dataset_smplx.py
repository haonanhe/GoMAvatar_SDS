...
def main(argv):
    ...
    for idx, ipath in enumerate(tqdm(img_paths)):
        ...
        # print(smplx_model.NUM_BODY_JOINTS)

        output = smplx_model(betas=torch.from_numpy(betas).unsqueeze(0), 
                             body_pose=torch.from_numpy(np.zeros(smplx_model.NUM_BODY_JOINTS * 3,).astype(np.float32)).unsqueeze(0),
                             return_verts=True,
                             return_full_pose=True)
        # vertices = output.vertices.detach().cpu().numpy().squeeze()
        tpose_joints = output.joints.detach().cpu().numpy().squeeze()

        # tpose_joints = tpose_joints[:num_joints, :]
        
        output = smplx_model(betas=torch.from_numpy(betas).unsqueeze(0), 
                             body_pose=torch.from_numpy(poses[:smplx_model.NUM_BODY_JOINTS * 3]).unsqueeze(0),
                             return_verts=True, 
                             return_full_pose=True)
        # vertices = output.vertices.detach().cpu().numpy().squeeze()
        ...
        # ### concat with lmks in original codes
        ...
        # num_joints = 54
        num_joints = full_pose.reshape(-1, 3).shape[0] # 55, the first one is global orient
        tpose_joints = tpose_joints[:num_joints, :]
        joints = joints[:num_joints, :]
    ...
    np.save(os.path.join(output_path, 'avg_betas.npy'), avg_betas)
    
    # smpl_model = SMPL(sex, model_dir=MODEL_DIR)
    # v, template_joints = smpl_model(np.zeros(72), avg_betas)
    ...
    smplx_model = smplx.create(MODEL_DIR, model_type='smplx',
                         ...
                         num_betas=10,
                         ext='npz')
    # output = smplx_model(betas=torch.from_numpy(betas), body_pose=torch.from_numpy(np.zeros(72)),
    #             return_verts=True)
    output = smplx_model(betas=torch.from_numpy(avg_betas).unsqueeze(0), 
                        body_pose=torch.from_numpy(np.zeros(smplx_model.NUM_BODY_JOINTS * 3, ).astype(np.float32)).unsqueeze(0),
                        return_verts=True,
                        return_full_pose=True)
    v = output.vertices.detach().cpu().numpy().squeeze()
    template_joints = output.joints.detach().cpu().numpy().squeeze()
    template_joints = template_joints[:num_joints, :]
    # v, template_joints = smplx_model(np.zeros(72), betas=avg_betas)
    weights = smplx_model.lbs_weights.cpu().numpy().squeeze()
