...
class Model(nn.Module):
	...
	def forward(self, K, E, cnl_gtfms, dst_Rs, dst_Ts, dst_posevec=None, canonical_joints=None,
		...
		############### random camera ###############
		# from zju_mocap dataset
		K = torch.tensor([[537.1407,    0.,      271.4171 ],
						 [  0.,      537.7115,  242.44179],
						 [  0.,        0.,        1.     ]]).unsqueeze(0)#.to(self.vertices.device)
		# perturb K
		K[:, 0, 0] += torch.randn(1)
		K[:, 1, 1] += torch.randn(1)
		K[:, 0, 2] += torch.randn(1)
		K[:, 1, 2] += torch.randn(1)
		K = K.to(self.vertices.device)
		### body
		body_cam_position = torch.randn(1, 3).to(self.vertices.device)
		body_centroid = torch.mean(vertices_observation.permute(1, 0), dim=0, keepdim=True)

		radius = 1.0
		body_cam_position_r = body_centroid + (body_cam_position - body_centroid) * (radius / torch.linalg.norm(body_cam_position - body_centroid, dim=1, keepdim=True))

		R = look_at_rotation(body_cam_position_r, at=body_centroid, up=((0, 1, 0),)).to(self.vertices.device) # (1, 3, 3)
		T = -torch.bmm(R, body_cam_position_r[:, :, None])[:, :, 0].to(self.vertices.device)
	
		E = torch.eye(4).to(self.vertices.device)
		E[:3, :3] = R
		E[:3, 3] = T
		E = E.unsqueeze(0)
		...
		### head
		# head_cam_position = torch.randn(1, 3).to(self.vertices.device)
		head_cam_position = body_cam_position
		head_centroid = vertices_observation.permute(1, 0)[9120, None, :] # nose
		
		radius = 0.3
		head_cam_position_r = head_centroid + (head_cam_position - head_centroid) * (radius / torch.linalg.norm(head_cam_position - head_centroid, dim=1, keepdim=True))

		head_R = look_at_rotation(head_cam_position_r, at=head_centroid, up=((0, 1, 0),)).to(self.vertices.device) # (1, 3, 3)
		head_T = -torch.bmm(head_R, head_cam_position_r[:, :, None])[:, :, 0].to(self.vertices.device)
	
		head_E = torch.eye(4).to(self.vertices.device)
		head_E[:3, :3] = head_R
		head_E[:3, 3] = head_T
		head_E = head_E.unsqueeze(0)

		head_albedos, head_masks = self.renderer(
			xyz_observation.unsqueeze(0).permute(0, 2, 1),
			appearance_feats.unsqueeze(0).permute(0, 2, 1),
			opacity,
			K, head_E, bg_col=bg_col,
			skeleton_info={'cov': cov_observation.unsqueeze(0)})
		...
		head_rgbs = head_albedos * shadings

		cv2.imwrite('./rgb.png', rgbs.detach().cpu().numpy()[0,:,:,::-1]*255)
		cv2.imwrite('./mask.png', masks.detach().cpu().numpy()[0,:,:]*255)
		cv2.imwrite('./head_rgb.png', head_rgbs.detach().cpu().numpy()[0,:,:]*255)
		cv2.imwrite('./head_mask.png', head_masks.detach().cpu().numpy()[0,:,:]*255)
		...
		return rgbs, masks, head_rgbs, head_masks, outputs
